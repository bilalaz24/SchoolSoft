<template>
  <div class="homework-container">
    ...
    <div v-for="homeworkObject in homework" :key="homeworkObject._id">
      <div
        @click="
          $router.push({
            ...
            }/subject/${homeworkObject.subject.toLowerCase()}/homework`,
      ...
      >
        <h3>
          Week {{ homeworkObject.dueWeek }} -
          {{ homeworkObject.title }}
        </h3>
        <p>
          {{ formatDateWithWeek(homeworkObject.dueDate) }}, w.
          {{ homeworkObject.dueWeek }}
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async getHomework() {
      try {
        const response = await this.$api.get(
          `/homework/authenticated/week/${this.getWeekOfYear(new Date())}`,
          {
            method: "GET",
            headers: {
    ...
    getWeekOfYear(date) {
      ...
      const pastDaysOfYear = (date - startOfYear) / 86400000 + daysOffset;
